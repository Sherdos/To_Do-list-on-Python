...
@con_db
def check(cur, user):
    cur.execute('update users set falled_task = falled_task+(select count(*) from tasks where date_at < current_date and user_id = %s) where id = %s', (user.id,user.id))
    cur.execute('delete from tasks where date_at < current_date and user_id = %s',(user.id,))
